...
def load_data(dataset):
    ...
    for i in range(len(names)):
        ...
        '''
        with open("dataset/ind.{}.{}".format(dataset, names[i]), 'rb') as rf:
            u = pkl._Unpickler(rf)
            u.encoding = 'latin1'
            cur_data = u.load()
    ...
    test_idx_reorder = parse_index_file(
        "dataset/ind.{}.test.index".format(dataset))
...
def cluster_acc(y_true, y_pred):
    ...
    l2 = list(set(y_pred))
    numclass2 = len(l2)
    # if num_class1 != numclass2:  调试amap不能运行注释
    #     print('error')
    #     return
...
def load_graph_data(dataset_name, show_details=False):
    ...
    features = np.load(load_path + "_feat.npy", allow_pickle=True)
    ...
    return adj, features, label,   torch.tensor(adj).float(), node_num, 0, 0, 0
    # return adj, features, label,0,0,0
...
def clustering(feature, true_labels, cluster_num):
    predict_labels, dis, initial = kmeans(X=feature, num_clusters=cluster_num, distance="euclidean",
                                          device="cuda")  # 根据diatance参数选择合适的距离计算函数
    acc, nmi, ari, f1 = eva(true_labels, predict_labels.numpy(), show_details=False)
    return 100 * acc, 100 * nmi, 100 * ari, 100 * f1, predict_labels.numpy(), dis, initial


def get_Q(z, cluster_num, Z_shape):
    cluster_layer = torch.nn.Parameter(torch.Tensor(cluster_num, Z_shape))
    torch.nn.init.xavier_normal_(cluster_layer.data)
    center0 = F.normalize(cluster_layer, p=2, dim=1)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    center0 = center0.to(device)
    q = 1.0 / (1.0 + torch.sum(torch.pow(z.unsqueeze(1) - center0, 2), 2) / 1)
    q = q.pow((1 + 1.0) / 2.0)
